...
describe Opponent do
  ...
  it 'blocks possible wins for every diagonale' do
    ...
    expect(next_move_for(x).in('  o   o  ')).to eq 4
    expect(next_move_for(x).in('  o o    ')).to eq 6
  end

  it 'wins when possible' do
    expect(next_move_for(x).in(' xx      ')).to eq 0
    expect(next_move_for(x).in('x x      ')).to eq 1
    expect(next_move_for(x).in('xx       ')).to eq 2

    expect(next_move_for(x).in('    xx   ')).to eq 3
    expect(next_move_for(x).in('   x x   ')).to eq 4
    expect(next_move_for(x).in('   xx    ')).to eq 5

    expect(next_move_for(x).in('       xx')).to eq 6
    expect(next_move_for(x).in('      x x')).to eq 7
    expect(next_move_for(x).in('      xx ')).to eq 8

    expect(next_move_for(x).in('   x  x  ')).to eq 0
    ...
    expect(next_move_for(o).in('    o o  ')).to eq 2
    expect(next_move_for(o).in('  o   o  ')).to eq 4
